        ...
        async function fetchProducts() {
            try {
                ...
                if (data.success && data.products && data.products.length > 0) {
                    ...
                    // Match predefined target handles first
                    const matched = [];
                    targetHandles.forEach(handle => {
                        const found = data.products.find(p => p.handle === handle || p.id === handle);
                        if (found && !matched.some(m => m.id === found.id)) {
                            matched.push(found);
                        }
                    });

                    // Fill the remaining slots with other available products to ensure 6 products
                    const remaining = data.products.filter(p => !matched.some(m => m.id === p.id));
                    const finalProducts = [...matched, ...remaining].slice(0, 6);
...
function ProductCircle({ product }) {
    ...
    const image = product.images?.edges?.[0]?.node || product.images?.[0] || (product.image?.url ? product.image : null) || (product.image_url ? { url: product.image_url, altText: shortTitle } : null);
