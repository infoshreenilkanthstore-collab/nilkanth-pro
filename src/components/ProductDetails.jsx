...
import { Star, CheckCircle2, User, Calendar, X, ChevronLeft, ChevronRight, ChevronDown, MessageSquarePlus, Ticket, Smartphone, Apple, MapPin, Truck, History, Sparkles, Info, Clock, ShieldCheck, ShoppingCart, Heart, Eye } from "lucide-react";
...
import { getPositiveWeight } from "@/lib/checkout";
...
export default function ProductDetails({ handle, initialProduct = null }) {
    ...
    const initialHandleRef = useRef(handle);
    useEffect(() => {
        // If handle changes while on this page, do a full page refresh/navigation as requested
        if (initialHandleRef.current && initialHandleRef.current !== handle) {
            if (typeof window !== "undefined") {
                window.location.href = `/products/${handle}`;
                return;
            }
        }
        initialHandleRef.current = handle;
    ...
    useEffect(() => {
        ...
        if (!trackViewContent()) {
            const interval = setInterval(() => {
                if (trackViewContent()) clearInterval(interval);
            }, 200);
    ...
    return (
        <div className="max-w-[1400px] mx-auto  font-nunito">
            ...
            <div className={`fixed bottom-16 md:bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-[50] transition-all duration-500 transform ${showSticky ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-[150%] opacity-0 pointer-events-none'}`}>
                <div className="max-w-[1400px] mx-auto px-6 py-2 flex items-center justify-between gap-4">
                    ...
                    <div className="flex flex-1 sm:flex-none items-center gap-2 md:gap-3">
                        {/* Variant Selector */}
                        {allVariants.length > 1 && (
                            <select
                                ...
                                className="h-11 px-3 bg-gray-50 border border-gray-100 rounded-xl text-[11px] font-bold text-gray-700 focus:outline-none focus:border-[#700b10] transition-colors cursor-pointer max-w-[120px] md:max-w-none"
                            >
                                {allVariants.map((v) => (
                        ...
                        <div className="flex items-center bg-gray-50 rounded-full border border-gray-100 overflow-hidden h-11 flex-shrink-0">
                            <button
                                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                                className="w-10 flex items-center justify-center text-gray-400 hover:text-[#700b10] transition"
                            >
                                −
                            </button>
...
//         if (typeof window !== "undefined") {
//             const checkoutValue = Number(priceAmountRaw) * quantity;
//             const checkoutCleanId = String(product?.id || "").replace(/^gid:\/\/shopify\/Product\//, "");
//             if (window.fbq) {
//                 const fbPayload = {
//                     content_name: product?.title,
//                     content_ids: [checkoutCleanId],
//                     content_type: 'product',
//                     num_items: quantity,
//                     currency: 'INR',
//                     value: checkoutValue
...
//                 console.log('🎯 [Meta Pixel] Tracked InitiateCheckout:', fbPayload);
//             }
//             if (window.gtag) {
//                 const gtagCheckoutPayload = {
//                     currency: 'INR',
//                     value: checkoutValue,
//                     items: [{
//                         item_id: checkoutCleanId,
//                         item_name: product?.title,
//                         price: Number(priceAmountRaw),
//                         quantity
//                     }]
//                 };
//                 window.gtag('event', 'begin_checkout', gtagCheckoutPayload);
//                 console.log('📊 [Google Tag / GA4] Tracked begin_checkout:', gtagCheckoutPayload);
//             }
//         }
...
//         const variantWeight = Number(selectedVariant?.weight || product?.variants?.edges?.[0]?.node?.weight || 0);
//         const prodWeight = Number(product?.weight || product?.min_weight || 0);

//         let finalWeight = 0;
//         let finalWeightUnit = "g";

//         if (isSingleVariant) {
//             // If only one variant, take directly from product root
//             finalWeight = prodWeight > 0 ? prodWeight : (variantWeight > 0 ? variantWeight : 0);
//             finalWeightUnit = product?.weight_unit || product?.weightUnit || selectedVariant?.weightUnit || selectedVariant?.weight_unit || "g";
//         } else {
//             // Multiple variants: prioritize variant weight
//             finalWeight = variantWeight > 0 ? variantWeight : (prodWeight > 0 ? prodWeight : 0);
//             finalWeightUnit = (variantWeight > 0 && (selectedVariant?.weightUnit || selectedVariant?.weight_unit))
...
//             if (match) {
//                 finalWeight = parseFloat(match[1]);
//                 const unit = match[2].toLowerCase();
//                 finalWeightUnit = (unit === "kg" || unit === "l") ? "kg" : "g";
//             }
//         }

//         const normalizedUnit = (() => {
//             const u = String(finalWeightUnit || "g").trim().toLowerCase();
...
//         return () => window.removeEventListener('scroll', handleScroll);
//     }, []);
//     // Meta Pixel (ViewContent) & Google Tag (view_item) Tracking
//     useEffect(() => {
//         if (!product?.id) return;
...
//             content_category: product.category || product.product_type || "Product",
//             content_ids: [cleanId],
//             content_type: 'product',
//             value: numericPrice,
//             currency: 'INR',
...
//                     price: numericPrice,
//                     item_category: product.category || product.product_type || "Product"
//                 }
//             ]
//         };
...
//             // 1. Meta Pixel (fbq)
//             if (!firedFbq && typeof window.fbq === "function") {
//                 try {
//                     window.fbq('track', 'ViewContent', pixelPayload);
//                     window.fbq('trackSingle', '1455730108844531', 'ViewContent', pixelPayload);
//                     console.log('🎯 [Meta Pixel] Tracked ViewContent (ID: 1455730108844531):', pixelPayload);
//                     firedFbq = true;
//                 } catch (e) {
...
//             // 2. Google Analytics / Google Tag (gtag)
//             if (!firedGtag && typeof window.gtag === "function") {
//                 try {
//                     window.gtag('event', 'view_item', gtagPayload);
//                     console.log('📊 [Google Tag / GA4] Tracked view_item:', gtagPayload);
//                     firedGtag = true;
//                 } catch (e) {
//                     console.error('Google Tag view_item error:', e);
//                 }
//             }
//         };
...
//         // Retry polling in case external scripts are initializing
//         const intervalId = setInterval(() => {
//             if (firedFbq && firedGtag) {
...
//     // Generate Complete Product JSON-LD Schema for Google & Rich Snippets
...
//     const productSku = selectedVariant?.sku || selectedVariant?.barcode || product?.sku || product?.barcode || variantIdClean || productIdClean;
...
//             if (u && !productImages.includes(u)) productImages.push(u);
//         });
//     }
//     if (productImages.length === 0 && mainImage) productImages.push(mainImage);

//     // Clean plain text description
//     const plainDescription = product?.seo_description || (product?.description
//         ? product.description.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()
//         : (product?.descriptionHtml ? product.descriptionHtml.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim() : product?.title || ""));

//     const currentProductUrl = typeof window !== "undefined" ? window.location.href : (product?.handle ? `https://nilkanthdham.org/product/${product.handle}` : "");

//     // Build variant offers
//     const variantOffers = (allVariants.length > 0 ? allVariants : [selectedVariant || {}]).map(v => {
//         const vQty = v?.inventory_quantity !== undefined ? v.inventory_quantity : v?.inventoryQuantity;
//         const vAvailable = v?.availableForSale !== false && (vQty === undefined || vQty > 0);
//         const vPrice = v?.price?.amount || v?.priceV2?.amount || v?.price || priceAmount || 0;
//         const vCurrency = v?.price?.currencyCode || v?.priceV2?.currencyCode || currency || "INR";
//         const vSku = v?.sku || v?.barcode || String(v?.id || "").replace(/^gid:\/\/shopify\/ProductVariant\//, "") || productSku;
//         const vImage = v?.image?.url || v?.image?.src || productImages[0];

//         return {
//             "@type": "Offer",
//             "name": v?.title ? `${product.title} - ${v.title}` : product.title,
//             "sku": vSku,
//             "price": Number(vPrice) || 0,
//             "priceCurrency": vCurrency,
//             "priceValidUntil": new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
//             "availability": vAvailable ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
...
//     // Build customer reviews for schema
//     const activeReviews = (reviews && reviews.length > 0) ? reviews : (Array.isArray(product?.reviews) ? product.reviews : []);
//     const reviewSchemaList = activeReviews.slice(0, 20).map(rev => ({
//         "@type": "Review",
//         "author": {
//             "@type": "Person",
//             "name": rev.customer_name || "Verified Customer"
//         },
//         "datePublished": rev.review_date ? rev.review_date.split('T')[0] : (rev.created_at ? rev.created_at.split('T')[0] : undefined),
//         "reviewBody": rev.description || rev.title || "Excellent",
...
//         "sku": productSku,
//         "mpn": productSku,
//         ...(product.category || product.product_type ? { "category": product.category || product.product_type } : {}),
//         ...(product.tags && Array.isArray(product.tags) && product.tags.length > 0 ? { "keywords": product.tags.join(", ") } : {}),
//         ...(product.vendor ? {
//             "brand": {
//                 "@type": "Brand",
//                 "name": product.vendor
...
//             "offers": {
//                 "@type": "AggregateOffer",
//                 "lowPrice": Number(product?.priceRange?.minVariantPrice?.amount || product.price || priceAmount || 0),
//                 "highPrice": Number(product?.priceRange?.maxVariantPrice?.amount || product.compare_at_price || priceAmount || 0),
//                 "priceCurrency": currency || "INR",
//                 "offerCount": variantOffers.length,
//                 "offers": variantOffers
//             }
//         } : {
//             "offers": variantOffers[0]
//         }),
//         ...(totalReviews > 0 ? {
//             "aggregateRating": {
...
//             }
//         } : {}),
//         ...(reviewSchemaList.length > 0 ? {
//             "review": reviewSchemaList
//         } : {})
//     };
