...
import { User, MapPin, Package, LogOut, Plus, Pencil, Trash2 } from "lucide-react";
...
function OrdersTab({ customerId, accessToken }) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/customer/orders?customerId=${customerId || ""}&accessToken=${accessToken || ""}`)
            .then(r => r.json())
            .then(d => {
    ...
    return (
        <div className="space-y-6">
            ...
            {orders.map((order) => (
                <div key={order.id} className="bg-white border border-yellow-100/50 shadow-sm rounded-3xl p-6 md:p-8">
                    <div className="flex flex-col md:flex-row justify-between md:items-center border-b border-gray-100 pb-4 mb-4 gap-4 md:gap-0">
                        <div>
                            <p className="font-bold text-gray-900 text-lg mb-1 hidden md:block">Order {order.orderNumber}</p>
                            <p className="font-bold text-gray-900 text-lg mb-1 md:hidden">Order <br />{order.orderNumber}</p>
                    ...
                    <div className="space-y-4 pt-2">
                        {order.items.map((item, idx) => (
                            <div key={item.id || idx} className="flex justify-between items-center text-sm md:text-base">
                                <p className="text-gray-700 font-medium flex items-center gap-3">
                                    <span className="w-8 h-8 rounded bg-gray-50 flex items-center justify-center text-gray-400 font-bold text-xs">{item.quantity}x</span>
                                    {item.title}
                                </p>
