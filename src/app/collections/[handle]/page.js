...
import { getProductWeightValues, buildWeightFilterGroup, normalizeWeightValue } from '@/lib/filterUtils';
...
export default async function CollectionPage({ params, searchParams }) {
    ...
    // 3. Weight Filter (Calculated from normalized options & variants)
    const weightFilterGroup = buildWeightFilterGroup(allProducts);
    if (weightFilterGroup) {
        availableFilters.push(weightFilterGroup);
    }
    ...
    // Weight
    if (sParams.weight) {
        const rawSelected = Array.isArray(sParams.weight) ? sParams.weight : [sParams.weight];
        const selectedWeights = rawSelected.map(w => normalizeWeightValue(w).toLowerCase()).filter(Boolean);
        filteredProducts = filteredProducts.filter(({ node }) => {
            const productWeights = getProductWeightValues(node).map(w => w.toLowerCase());
