...
async function getFullCartDetails(rawCart) {
  ...
  return rawCart
    .map(item => {
      ...
      const variantNode = details.variants?.edges?.find(edge => edge.node.id === item.variantId || getNumericId(edge.node.id) === getNumericId(item.variantId))?.node 
                       || details.variants?.edges[0]?.node;

      const isSingleVariant = (details.variants?.edges?.length || 0) <= 1;
      const variantWeight = Number(variantNode?.weight || 0);
      const productWeight = Number(details?.weight || details?.min_weight || 0);
      ...
      if (isSingleVariant) {
        ...
        finalWeight = productWeight > 0 ? productWeight : (variantWeight > 0 ? variantWeight : (Number(item.weight) || 0));
        finalWeightUnit = details?.weight_unit || details?.weightUnit || item.weightUnit || item.weight_unit || variantNode?.weightUnit || "g";
      } else {
        // Multiple variants: prioritize variant weight
        finalWeight = variantWeight > 0 ? variantWeight : (productWeight > 0 ? productWeight : (Number(item.weight) || 0));
        finalWeightUnit = (variantWeight > 0 && variantNode?.weightUnit)
          ? variantNode.weightUnit
          : (details?.weight_unit || details?.weightUnit || item.weightUnit || item.weight_unit || "g");
      }

      if (finalWeight === 0) {
        const titleStr = variantNode?.title || details?.title || item.title || item.variantTitle || "";
      ...
      return {
        ...item,
        title: details.title || item.title || "Unknown Product",
        variantTitle: variantNode?.title || item.variantTitle || "",
        image: details.images?.edges[0]?.node?.url || item.image || "",
        price: variantNode?.price?.amount || details.variants?.edges[0]?.node?.price?.amount || item.price || "0.00",
        weight: finalWeight,
        weightUnit: normalizeWeightUnit(finalWeightUnit)
      };
    })
...
export async function POST(req) {
  try {
    const { customerId, productId, variantId, qty, weight, weightUnit, action, cart: newFullCart } = await req.json();
    if (!customerId) return NextResponse.json({ success: false, error: "Missing customerId" }, { status: 400 });
    ...
    if (newFullCart && Array.isArray(newFullCart)) {
      ...
      finalCart = newFullCart.map(i => ({
        ...
        weightUnit: normalizeWeightUnit(i.weightUnit)
      }));
    } else {
      ...
      if (action === "add") {
        if (index === -1) cart.push({ productId, variantId, qty: Number(qty || 1), weight: Number(weight || 0), weightUnit: normalizeWeightUnit(weightUnit) });
