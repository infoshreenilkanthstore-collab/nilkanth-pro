...
export default function MarketingScriptInjector() {
  ...
  useEffect(() => {
    async function fetchScripts() {
      ...
      try {
        ...
        console.log(`[MarketingScript] Fetching scripts for route: ${pathname} and /all`);
        ...
        console.log('[MarketingScript] Combined API Response:', combinedScripts);
  ...
  useEffect(() => {
    ...
    scripts.forEach(item => {
      console.log(`[MarketingScript] Processing script (Placement: ${item.placement})`, item);
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = item.script_content;

      const parsedScripts = Array.from(tempDiv.querySelectorAll('script'));
      console.log(`[MarketingScript] Found ${parsedScripts.length} <script> tags and ${tempDiv.children.length - parsedScripts.length} other tags`);
      ...
      parsedScripts.forEach(parsedScript => {
        ...
        if (parsedScript.textContent) {
          console.log(`[MarketingScript] About to execute script code: "${parsedScript.textContent}"`);
        ...
        if (item.placement === 'head') {
          ...
          console.log('[MarketingScript] Injected script into <head> successfully!');
        } else {
          document.body.appendChild(newScript);
          addedNodes.push(newScript);
          console.log('[MarketingScript] Injected script into <body> successfully!');
        }
      });
      ...
      Array.from(tempDiv.children).forEach(node => {
        if (node.tagName.toLowerCase() !== 'script') {
          ...
          console.log(`[MarketingScript] Injected <${node.tagName.toLowerCase()}> into ${item.placement}`);
        }
      });
    });
